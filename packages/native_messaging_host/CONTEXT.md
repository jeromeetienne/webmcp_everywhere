...
## Rules
- This package is a program and an entry point at once, and the two are reached differently. `bin/webmcp_native_host.sh` and `tools/package_release.ts` name `src/webmcp_native_host.ts` by path, because a program Chrome starts is a path, and the launcher works that path out from its own location rather than holding one. Everything imported goes through `src/index.ts` under the single `"."` key. Moving `src/webmcp_native_host.ts` means editing the launcher; moving the repository means running `npm run install:host` again.
...
- `bin/webmcp_native_host.sh` stays tracked in git. [Issue #4](https://github.com/jeromeetienne/webmcp_everywhere/issues/4) asked for it to leave, because the file then held absolute paths true only on the machine that generated it, so a checkout or a branch switch rewrote what Chrome would start. It works every path out from its own location now, which makes it source rather than a generated file.
