...
## Rules
- This package is a program and an entry point at once, and the two are reached differently. `bin/native_messaging_host.sh` and `tools/webmcp_everywhere/package_release.ts` name `src/webmcp_native_host.ts` by path, because a program Chrome starts is a path, and the launcher works that path out from its own location rather than holding one. Everything imported goes through `src/index.ts` under the single `"."` key. Moving `src/webmcp_native_host.ts` means editing the launcher; moving the repository means running `npm run install:host` again.
