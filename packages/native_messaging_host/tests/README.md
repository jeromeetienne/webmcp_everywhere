...
These runners cover the path [`/packages/native_messaging_host`](../README.md) serves: in at the HTTP endpoint, across native messaging to the extension, into the page, and back out again — and the file that tells an agent where that endpoint is.
...
```bash
node --test --test-concurrency=1 packages/native_messaging_host/tests/*.test.ts
```
