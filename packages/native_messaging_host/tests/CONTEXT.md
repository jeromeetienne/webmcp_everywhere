# Directory Context: `/packages/native_messaging_host/tests`

## Purpose
The runners covering the path [the package this folder sits in](../CONTEXT.md) serves: in at the HTTP endpoint, across native messaging to the extension, into the page and back — and the file that tells an agent where that endpoint is.
...
- Command to check this folder: `node --test --test-concurrency=1 packages/native_messaging_host/tests/*.test.ts`
...
## Background
- The host program these two check is [the package this folder sits in](../CONTEXT.md), and why it exists at all is [why_a_native_messaging_host.md](../../../docs/why_a_native_messaging_host.md).
...
- This folder was `tests/delivery_path/` until [issue #25](https://github.com/jeromeetienne/webmcp_everywhere/issues/25), and `tests/native_messaging_host/` until [issue #28](https://github.com/jeromeetienne/webmcp_everywhere/issues/28) moved each such folder inside the folder it checks. Issue #25 also says why `native_host.test.ts` sits here rather than with the extension: it enters at the HTTP endpoint, and splitting it from `endpoint_file.test.ts` would lose the pairing above.
