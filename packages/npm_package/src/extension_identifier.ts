 ...
 * to read. Generating the key pair in the first place is `tools/generate_extension_key.ts`, which runs
...
export class ExtensionIdentifier {
	...
	static fromManifest(manifestPath: string): string {
		...
		if (manifest.key === undefined) {
			throw new Error(
				`${manifestPath} has no key; run "node tools/generate_extension_key_entry.ts" first`,
			);
		}
