# Directory Context: `/packages/webmcp_everywhere/tests`

## Purpose
The runners covering what [the package this folder sits in](../CONTEXT.md) installs, in the order a user meets it: the host manifest file written into Chrome, the package npmjs carries, and a release driven with no repository under it.
...
- Command to check this folder: `node --test --test-concurrency=1 packages/webmcp_everywhere/tests/*.test.ts`
...
- This folder was `tests/installation/` until [issue #25](https://github.com/jeromeetienne/webmcp_everywhere/issues/25) named every runner folder after the source folder it checks, and `tests/webmcp_everywhere/` until [issue #28](https://github.com/jeromeetienne/webmcp_everywhere/issues/28) moved each such folder inside the folder it checks. Every product file these runners reach is now `../src/`.
