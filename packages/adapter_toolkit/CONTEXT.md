...
- This package imports nothing, not `@webmcp_everywhere/adapter_format`, not anything under `contribs/`, and not any adapter. It is bundled into a main-world content script on every covered page, so it stays small enough that its size never becomes a reason to argue about it.
