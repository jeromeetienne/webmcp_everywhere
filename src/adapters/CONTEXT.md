...
- An adapter imports types from `../../adapter_format/` and nothing else. It never imports another adapter, and it never imports from `extension/`.
- Every adapter sets a `yieldCondition`. An adapter that cannot stand down when the site ships its own tools is not finished.
- Adapters are added to `src/extension/adapter_registry.ts` by hand. There is no automatic discovery, because a build that silently picks up a new file is a build that silently ships one.
