...
- An adapter imports types from `../../adapter_format/` and helpers from `@webmcp_everywhere/adapter_toolkit`, and nothing else. It never imports another adapter, and it never imports from `chrome_extension/`.
- A helper that any second site would also need belongs in `packages/adapter_toolkit/`, not here. What stays here is this site's own figures and its own selectors: how long it takes to settle, where it keeps its state, which element means what.
