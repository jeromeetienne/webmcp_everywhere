...
- Adapters are added to `src/chrome_extension/adapter_registry.ts` by hand. There is no automatic discovery, because a build that silently picks up a new file is a build that silently ships one.
