...
## Purpose
Everything that ships and has no `package.json` of its own: the adapters, and the browser extension that loads them. What an adapter is written against, the native messaging host that serves an agent, and the package npmjs carries all have a `package.json` of their own and live in `packages/`. Build tooling lives in `tools/`, and verification lives in `tests/`.
...
- Nothing under `src/` reaches the network at runtime. Adapters read and drive their own page and nothing else, and the one program that listens on a socket is `packages/native_messaging_host/`, on the loopback interface only.
