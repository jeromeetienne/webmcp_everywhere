...
## Rules
- Nothing here imports from `tools/` or from `tests/`. `node --test tests/source_boundary.test.ts` refuses any relative import that leaves `src/`, which is what keeps build tooling and verification code from drifting back in.
...
- `adapter_format/` never imports from `chrome_extension/` or from `site_adapters/`, so an adapter can be checked without a browser. `adapter_toolkit/` imports nothing at all.
