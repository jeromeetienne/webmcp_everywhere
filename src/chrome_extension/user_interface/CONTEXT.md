...
## Purpose
The popup a person opens from the toolbar: it shows which adapter matched the current tab, which tools an agent can currently call, and lets a person grant acting, pull the kill switch, and clear an injection sighting.
...
- `popup.ts`: Reads the current tab, renders the page, and writes the grant back.
