...
- `loaded_adapter_store.ts`: `LoadedAdapterStore` — the folder of adapters `npm run load-adapter` writes, read once per connection.
...
- `bin/webmcp_native_host.sh` stays tracked in git. [Issue #4](https://github.com/jeromeetienne/webmcp_everywhere/issues/4) asked for it to leave, because the file at that path then held absolute paths true only on the machine that generated it, so a checkout or a branch switch rewrote what Chrome would start. It is now a shell script that works every path out from its own location, which makes it source rather than a generated file, and the reason to untrack it is gone.
