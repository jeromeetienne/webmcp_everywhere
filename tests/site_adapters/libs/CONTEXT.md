...
- `todomvc_result_types.ts` is imported from outside this folder as well, by `tests/code_from_outside/injection_defence.test.ts` and by `tests/devtools_protocol_bridge/webmcp_bridge.test.ts`, because both attack or drive the same TodoMVC page.
