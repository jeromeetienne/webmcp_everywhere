...
These runners cover what [`/contribs/chrome_extension`](../../contribs/chrome_extension/README.md) enforces against code and content this repository did not write: a hostile page, and an adapter somebody else wrote. Both reach the browser through the extension, which is the only place that decides what a page is allowed to do.
...
```bash
node --test --test-concurrency=1 tests/chrome_extension/*.test.ts
```
