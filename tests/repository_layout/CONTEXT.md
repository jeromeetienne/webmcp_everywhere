...
- A runner here reads the repository off disk rather than naming its contents in a list, so a folder added tomorrow is checked without this folder being edited. `SourceBoundaryTest._collectProductRoots` reads `packages/`; `AdapterRegistrySyncTest` reads `src/site_adapters/`.
