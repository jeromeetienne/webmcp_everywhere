...
- `endpoint_file.test.ts`, `native_host_install.test.ts`, and `source_boundary.test.ts` are the three that start no browser. The subject of `endpoint_file.test.ts` is the host process and the file it writes, and the fault it covers is a host whose standard input never reached its end, which a browser cannot be asked for. It still starts the real host over a real pipe, into a throwaway `WEBMCP_EVERYWHERE_STATE_DIR`.
