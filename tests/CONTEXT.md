...
- Nothing here writes into the browser the user installed, which [issue #4](https://github.com/jeromeetienne/webmcp_everywhere/issues/4) refuses: `installation/native_host_install.test.ts` passes `isEverydayChromeCovered: false`, and `installation/webmcp_everywhere.test.ts` sets `HOME` elsewhere.
