...
These runners cover what [`/packages/webmcp_everywhere`](../../packages/webmcp_everywhere/README.md) installs, in the order a user meets it: the host manifest file written into Chrome, the package npmjs carries, and a release driven with no repository under it.
...
```bash
node --test --test-concurrency=1 tests/webmcp_everywhere/*.test.ts
```
