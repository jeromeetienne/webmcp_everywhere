# Directory Context: `/tests/webmcp_everywhere`

## Purpose
The runners covering what [`/packages/webmcp_everywhere`](../../packages/webmcp_everywhere/CONTEXT.md) installs, in the order a user meets it: the host manifest file written into Chrome, the package npmjs carries, and a release driven with no repository under it.
...
- Command to check this folder: `node --test --test-concurrency=1 tests/webmcp_everywhere/*.test.ts`
...
- This folder was `tests/installation/` until [issue #25](https://github.com/jeromeetienne/webmcp_everywhere/issues/25) named every runner folder after the source folder it checks. All three runners here import only from `packages/webmcp_everywhere/src/`.
