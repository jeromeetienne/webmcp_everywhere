# Directory Context: `/tests/delivery_path`

## Purpose
The runners covering the path an agent's request takes: in at the HTTP endpoint, across native messaging to the extension, into the page and back — and the file that tells an agent where that endpoint is.
...
- Command to check this folder: `node --test --test-concurrency=1 tests/delivery_path/*.test.ts`
