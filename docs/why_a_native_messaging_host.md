...
It is kept because it is the smallest way to tell an adapter fault apart from a delivery fault when `node --test packages/native_messaging_host/tests/native_host.test.ts` fails. It is not the product for three reasons.
