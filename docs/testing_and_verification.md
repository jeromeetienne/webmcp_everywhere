...
| [`tests/delivery_path/native_host.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/delivery_path/native_host.test.ts) | The real delivery path, from the HTTP endpoint through to the page |
...
| [`tests/code_from_outside/loaded_adapter.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/code_from_outside/loaded_adapter.test.ts) | That an adapter written outside this repository is checked, installed, and run with no rebuild |
| [`tests/delivery_path/endpoint_file.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/delivery_path/endpoint_file.test.ts) | That `endpoint.json` always names a host that is really listening |
| [`tests/installation/native_host_install.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/installation/native_host_install.test.ts) | That installing announces every file first, and that uninstalling removes every one of them |
| [`tests/code_from_outside/injection_defence.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/code_from_outside/injection_defence.test.ts) | Writes hostile content onto the page and attacks through it |
...
| [`tests/installation/packaged_release.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/installation/packaged_release.test.ts) | That a packaged release installs and runs with no repository under it |
...
`npm run test:no_browser` names the six runners that start no browser: [`tests/repository_layout/adapter_registry_sync.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/repository_layout/adapter_registry_sync.test.ts), [`tests/delivery_path/endpoint_file.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/delivery_path/endpoint_file.test.ts), [`tests/installation/native_host_install.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/installation/native_host_install.test.ts), [`tests/installation/webmcp_everywhere.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/installation/webmcp_everywhere.test.ts), [`tests/repository_layout/source_boundary.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/repository_layout/source_boundary.test.ts), and [`tests/repository_layout/workspace_packages.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/repository_layout/workspace_packages.test.ts). Those six, plus `npm run typecheck` and `npm run build`, are what [`.github/workflows/checks_without_a_browser.yml`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/.github/workflows/checks_without_a_browser.yml) runs on every pull request, and they answer in about a minute.
...
**[`release.yml`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/.github/workflows/release.yml)** packages the extension and the native messaging host into one folder, drives that folder in a real Chrome with `node --test tests/installation/packaged_release.test.ts`, and only then attaches the archive to the release for the tag. Nothing is published that has not been started.
...
- **You changed the extension or the native messaging host.** Run `node --test tests/delivery_path/native_host.test.ts`, which is the only runner covering the real delivery path.
- **You changed how an adapter is loaded, checked, or registered.** Run `node --test tests/code_from_outside/loaded_adapter.test.ts`, which is the only runner covering the path from a folder outside this repository to a running tool.
- **You changed the packaging, the launcher, or anything the host reads from disk.** Run `node --test tests/installation/packaged_release.test.ts`, with your everyday Chrome closed. It is the only runner that proves the host works with no repository under it.
- **`node --test tests/delivery_path/native_host.test.ts` fails and you cannot tell why.** Run the adapter's own runner. If that passes, the adapter is fine and the fault is in delivery. `node --test tests/devtools_protocol_bridge/webmcp_bridge.test.ts` narrows it further, because the bridge reaches the page without the extension or the host in the way.
- **You touched how the host takes its port, stops, or writes `endpoint.json`.** Run `node --test tests/delivery_path/endpoint_file.test.ts`.
- **You touched what the installation writes into Chrome, or what the uninstallation takes back out.** Run `node --test tests/installation/native_host_install.test.ts`.
- **You touched the framing or the injection watch.** Run `node --test tests/code_from_outside/injection_defence.test.ts`.
...
[`tests/delivery_path/endpoint_file.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/delivery_path/endpoint_file.test.ts), [`tests/installation/native_host_install.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/installation/native_host_install.test.ts), and [`tests/repository_layout/source_boundary.test.ts`](https://github.com/jeromeetienne/webmcp_everywhere/blob/main/tests/repository_layout/source_boundary.test.ts) start no Chrome. The subject of `endpoint_file.test.ts` is the host process and the file it writes, and the fault that hid the longest in that file was a host whose standard input never reached its end — a state a browser cannot be asked for, but a named pipe held open by another process reproduces exactly. Nothing is stood in for. The hosts are the real program, started over a real pipe the way Chrome starts them, holding a real port and writing the real file to a throwaway directory named by `WEBMCP_EVERYWHERE_STATE_DIR`, so the checks never disturb the host you are really using.
