...
- `workflows/release.yml`: Packages a release, drives it in a real Chrome, and only then attaches the archive to the release for a `v*` tag.
...
- `release.yml` publishes nothing that has not been started. The archive is attached only after `tests/packaged_release.test.ts` has copied the release out of the repository and driven it in a real Chrome.
