...
- Everything every adapter is written against is one package rather than a folder here: `@webmcp_everywhere/site_adapter_lib`, holding both the shape and the framing an adapter conforms to and the waiting and driving helpers it shares. It imports nothing from `contribs/`, so an adapter can be checked without a browser and an author outside this repository can install it.
- Anything an adapter needs at runtime must be cheap. Validation belongs in `packages/site_adapter_lib/tools/`, because bundling the schema library into a main-world content script cost about 150 kilobytes on every page for no protection at all.
