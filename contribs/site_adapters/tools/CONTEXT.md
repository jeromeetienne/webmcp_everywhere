...
- `load_adapter.ts` runs the same checks the build runs, in [`packages/site_adapter/tools/`](../../../packages/site_adapter/tools/CONTEXT.md), and prints every tool with its permission class first: installing somebody else's code into your own logged-in sessions is done on purpose.
