...
export type AdapterNames = {
	...
	/** The verification runner under the adapter folder's own `tests/`, such as `example.test.ts`. */
...
export class NewAdapter {
	/**
	 * Names the command that runs one adapter's verification runner, which lives in that adapter's own folder.
	 *
	 * @param names - Every name worked out from the address.
	 * @returns The command, as a person would type it.
	 */
	static runnerCommandFor(names: AdapterNames): string {
		return `node --test contribs/site_adapters/${names.siteSlug}/tests/${names.runnerFileName}`;
	}
	...
	static async run(address: string, shortName?: string): Promise<{ names: AdapterNames; written: string[] }> {
		...
		const files: Array<[string, string]> = [
			...
			[
				Path.join(folder, 'tests', names.runnerFileName),
	...
	static _renderAdapter(names: AdapterNames): string {
		...
		return [
			...
			' *',
			' * For waiting and for driving, use `@webmcp_everywhere/site_adapter_lib`: `PageWaiting` for waiting on the page,',
	...
	static _renderRunner(names: AdapterNames): string {
		...
		return [
			...
			"import { LivePageHarness } from '../../../../tests/libs/live_page_harness.ts';",
			'',
	...
	static _renderContext(names: AdapterNames): string {
		const runnerCommand = NewAdapter.runnerCommandFor(names);
		return [
			...
			`- \`tests/${names.runnerFileName}\`: the verification runner, which drives the live site and asserts against what it reads back.`,
			`- Command to check this adapter against the live site: \`${runnerCommand}\``,
			'',
			'## Rules',
			...
			'- Nothing here imports from another adapter, or from anything under `chrome_extension/`. Types come from ' +
				'`@webmcp_everywhere/site_adapter_lib`, which also carries the waiting and driving helpers.',
	...
	static _renderReadme(names: AdapterNames): string {
		return [
			...
			'',
			`Last checked on ${NewAdapter._today()}, with \`${NewAdapter.runnerCommandFor(names)}\`.`,
			'',
		].join('\n');
	}
...
if (import.meta.filename === process.argv[1]) {
	...
	console.log(`  3. ${NewAdapter.runnerCommandFor(result.names)}`);
