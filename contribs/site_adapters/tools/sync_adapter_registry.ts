...
export type DiscoveredAdapter = {
	...
	/** Its verification runner, in that adapter folder's own `tests/`, such as `caniuse.test.ts`. */
...
export class SyncAdapterRegistry {
	...
	/** The one folder under `contribs/site_adapters/` that is not an adapter. */
	static readonly TOOLS_DIR_NAME = 'tools';
	...
	/**
	 * Lists the adapter file of every adapter folder under `contribs/site_adapters/`.
	 *
	 * `tools/` sits beside the adapter folders and is not one: it holds `new_adapter.ts`,
	 * `load_adapter.ts` and `unload_adapter.ts`, three file names ending the way an adapter file ends.
	 *
	 * @returns One path per adapter, relative to the repository root, ordered by folder name.
	 * @throws When an adapter folder holds no adapter file, or holds more than one.
	 */
	static _adapterFiles(): string[] {
		const folders = Fs.readdirSync(siteAdaptersDir, {
			...
			.filter((name) => name !== SyncAdapterRegistry.TOOLS_DIR_NAME)
			.sort();
