...
- Issue #1 expects a content security policy to stop adapters reaching the network. It cannot: a main-world script runs under the page's own policy, not the extension's. The check in `packages/site_adapter_lib/tools/permission_audit.ts`, which runs before an adapter is bundled or installed, is what enforces the rule instead.
