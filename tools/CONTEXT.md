...
- Every way of installing something has a way back, as easy to find as the way in: `unload_adapter.ts`, `uninstall_native_host.ts`, `npx webmcp_everywhere uninstall`.
...
- A packaged release carries its own launcher, bundled host and manifest template, and `InstallNativeHost` is pointed at them. One reaching for `src/` would pass every check here and fail for every user.
...
## Background
- Every rule about launching Chrome is a failure that happened in [issue #2](https://github.com/jeromeetienne/webmcp_everywhere/issues/2); none report an error. The everyday Chrome and the announcement come from [issue #4](https://github.com/jeromeetienne/webmcp_everywhere/issues/4).
